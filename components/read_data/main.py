...
def main(
    ...
    if len(annot_list)==0:
        log.error("No data following the introduced criteria were found.")
        return RuntimeError("No data following the introduced criteria were found.")
    # If files were found, generate output
    pd.DataFrame({'paths':annot_list}).to_csv(os.path.join(output_path, 'output.csv'), index=False)
